...
(function() {
    ...
    console.log('Map element exists:', document.getElementById('world-map') !== null);
    ...
    if (typeof L !== 'undefined') {
        ...
        const riskMap = {!! json_encode($riskMap) !!};
        console.log('Risk map data loaded, entries:', Object.keys(riskMap).length);
            ...
            .then(world => {
                ...
                L.geoJSON(world, {
                    ...
                    onEachFeature: function(feature, layer) {
                        ...
                        if (riskMap[code]) {
                            layer.bindTooltip(
                                `<b>${feature.properties.name}</b><br>Risk: ${riskMap[code].level}<br>Score: ${riskMap[code].score}`
                            );
                        }
                    }
                }).addTo(map);
        ...
        console.log('Ports data loaded:', ports.length, 'ports');
        ...
        ports.forEach(function(port) {
            ...
            const marker = L.circleMarker([lat, lng], {
                radius: port.outflows > 100000 ? 10 : port.outflows > 30000 ? 8 : 6,
                color: color,
                fillColor: color,
                fillOpacity: 0.9
            });
            ...
            marker.bindPopup(`
                <div style="width:260px;font-family:Arial,sans-serif;">
                    <div style="font-size:18px;font-weight:bold;margin-bottom:10px;color:#0f172a;">
                        🚢 ${port.name}
                    </div>
                    <table style="width:100%;font-size:13px;border-collapse:collapse;">
                        <tr><td style="padding:6px 0;"><b>Country</b></td><td style="text-align:right">${port.country?.name ?? '-'}</td></tr>
                        <tr><td style="padding:6px 0;"><b>UNLOCODE</b></td><td style="text-align:right">${port.locode ?? '-'}</td></tr>
                        <tr><td style="padding:6px 0;"><b>Status</b></td><td style="text-align:right">
                            <span style="background:#22c55e;color:white;padding:3px 8px;border-radius:20px;font-size:11px;">
                                ${port.status ?? 'Operational'}
                            </span>
                        </td></tr>
                        <tr><td style="padding:6px 0;"><b>Outflows</b></td><td style="text-align:right">${Number(port.outflows).toLocaleString()}</td></tr>
                        <tr><td style="padding:6px 0;"><b>Latitude</b></td><td style="text-align:right">${lat.toFixed(4)}</td></tr>
                        <tr><td style="padding:6px 0;"><b>Longitude</b></td><td style="text-align:right">${lng.toFixed(4)}</td></tr>
                    </table>
                    <hr style="margin:12px 0;">
                    <a href="/ports/${port.id}" style="display:block;width:100%;padding:10px;background:#2563eb;color:white;text-align:center;border-radius:8px;text-decoration:none;">
                        Open Detail
                    </a>
                </div>
            `);
        ...
        console.log('Added', validPorts, 'valid ports to cluster');
        
        map.addLayer(cluster);
        console.log('Cluster layer added to map');
